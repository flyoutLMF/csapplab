...
void eval(char *cmdline) 
{
    ...
    if(sigfillset(&mask_all) < 0)//设置阻塞集合
    ...
    strcpy(buf, cmdline);
    bg = parseline(buf, argv);//得到argv以及是否是bg
...
void do_bgfg(char **argv) 
{
    ...
    if(sigaddset(&mask, SIGCHLD) < 0)//设置阻塞集合
        unix_error("sigset error");
    if(sigprocmask(SIG_BLOCK, &mask, &prev) < 0)//在调用do_fgbg时阻塞SIGCHLD信号,防止调用过程中接收到SIGCHLD信号而发生不一致
            unix_error("sigset error");
...
void sigchld_handler(int sig) 
{
    ...
    while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)//循环等待所有的子进程的终止或停止
    {
