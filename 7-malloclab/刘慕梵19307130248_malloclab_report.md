<h1 style="font-family: '黑体'; text-align: center; font-weight: 700; color: skyblue;">malloclab_report</h1>
...
![image-20210519175438487](C:\Users\lalala\AppData\Roaming\Typora\typora-user-images\image-20210519175438487.png)

![image-20210519175448904](C:\Users\lalala\AppData\Roaming\Typora\typora-user-images\image-20210519175448904.png)
...
static int Check(void* bp, int mod);//check函数
/* 
 * mm_init - initialize the malloc package.
 * 堆的组织方式为分离式空闲链表，一共设置了9个大小的空闲块链表，分别为32B以下，64B以下，128B以下，256B以下，512B以下，   
 * 1024B以下，2048B以下，4096B以下，以及4096B以上，除了4096B以上（第8级）的和32B以下（第0级）的，第i级链表中空闲块大小b为 
 * 2^(i+4) < b <= 2^(i+5)。初始化链表有12个块，第0到8块储存第0到8级链表的表头指针，第9、10块为序言块，11块为堆尾块，
 * 用来标志堆开头和结束。heap_listp指向第0级链表，heap_tailp指向第一个序言块。分配块与空闲块有4字节的块头与块尾，
 * 空闲块在块头后面还有两个4字节的指针，分别指向上一个空闲块与下一个空闲块，空闲块按块大小排序。
 */
int mm_init(void)
{
    char *ptr;
...
static char* find_list_root(size_t size)//从对应大小得到对应等级链表的根
{
    ...
    return heap_listp + WSIZE * level;//对应等级链表的根
}
...
static int Check(void* bp, int mod)
{
    switch(mod)
    {
        case 0://返回的地址检查，如果不为8对齐，则出错
        {
            if((unsigned int)bp != (unsigned int)bp & ~0x7)
            {
                fprintf(stderr, "not 8 Bytes align at address: %p\n", bp);
                return 1;
            }
            break;
        }
        case 1://检查bp是否标志设置为0
        {
            ...
            {
                fprintf(stderr, "free block(%p) did not set alloc free\n", bp);
                return 1;
            }
            break;
        }
        case 2://检查是否空闲链表中每一块标志都设置为0
        {
            for(char *headptr = heap_listp; headptr != heap_tailp; headptr += WSIZE)
            {
                for(char* fbp = GET(headptr); fbp != NULL; fbp = GET(NEXT_FREE(fbp)))
                {
                    if(GET_ALLOC(HDRP(fbp)) || GET_ALLOC(FTRP(fbp)))
                    {
                        fprintf(stderr, "free block(%p) in list did not set alloc free\n", fbp);
                        return 1;
                    }
                }
            }
        ...
        case 3://检查bp是否加入到空闲块链表
        {
            if(GET_ALLOC(HDRP(bp)))
                return 0;
            char *headptr = get_list_root(bp);
            for(char *cmpbp = GET(headptr); cmpbp != NULL; cmpbp = GET(NEXT_FREE(cmpbp)))
            {
                if(cmpbp == bp)
                    return 0;
            }
            fprintf(stderr, "free block(%p) is not in list\n", bp);
            return 1;
        }
        case 4://检查是否每个空闲块都加入到了空闲链表
        {
            for(char *nowbp = heap_tailp + 3 * WSIZE; GET_SIZE(HDRP(nowbp)) != 0; nowbp = NEXT_BLKP(nowbp))
            {
                if(GET_ALLOC(HDRP(nowbp)))
                    continue;
                char *headptr = get_list_root(nowbp);
                for(char *cmpbp = GET(headptr); cmpbp != NULL; cmpbp = GET(NEXT_FREE(cmpbp)))
                {
                    if(cmpbp == nowbp)
                        return 0;
        ...
        case 5://用于realloc中，打印该地址下的内容
        {
            static int realloc_checktimes = 0;
            printf("%d:\n",realloc_checktimes);
            realloc_checktimes++;
